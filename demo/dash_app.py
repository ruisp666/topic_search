...
def display_topic_time(section):
    data = requests.get(url=os.path.join(URL_API,'get_topics_time'), params={'top_n': '5', 'freq': '1Y'}).json()
    df = pd.read_json(data['Section1'], orient='records')
    df['Timestamp'] = pd.to_datetime(df['Timestamp'])
    df = df.set_index('Timestamp')
    fig = px.line(data_frame=df, y='Frequency', color='Topic')
    fig.update_layout(
